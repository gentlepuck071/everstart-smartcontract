...
describe('hardCapExceededCase', async function() {
  describe('hardCapExceededCase', async function() {
    ...
    it('Claim reward', async function() {
      ...
      expect(wallet3BalanceAfter - wallet3BalanceBefore).to.eq(supplyAmount);
    });
    ...
    it('Withdraw EVERs', async function() {
      ...
      const amountOfWithdraw = locklift.utils.convertCrystal(10, 'nano');
    ...
    it('Return deposit sulpur', async function() {
      ...
      const amountOfWithdraw = launchPoolDetailsBefore.totalRaised - locklift.utils.convertCrystal(10, 'nano');
      ...
      expect(account3BalanceAfter - account3BalanceBefore).to.gt(Number(amountOfWithdraw - locklift.utils.convertCrystal(0.5, 'nano'))).and.lt(Number(amountOfWithdraw));
      expect(launchPoolBalanceBefore - launchPoolBalanceAfter).to.equal(Number(amountOfWithdraw));
      expect(launchPoolDetailsBefore.totalReturned.toNumber()).to.equal(0);
      expect(launchPoolDetailsAfter.totalReturned.toNumber()).to.equal(Number(amountOfWithdraw));
    });
  });
});
