...
describe('createWalletForClaim', async function() {
  describe('createWalletForClaim', async function() {
    ...
    it('Claim reward', async function() {
      ...
      const eventsBefore = (
        await locklift.ton.client.net.query_collection({
            ...
            filter: {
              src: {
                eq: launchPool.address
              },
            },
            result: 'body id src created_at msg_type msg_type_name',
          }
        )
      ).result;
      ...
      const launchPoolParticipationBefore = (await locklift.ton.getBalance(launchPoolParticipation.address)).toNumber();
      const launchPoolBefore = (await locklift.ton.getBalance(launchPool.address)).toNumber();
      const account3Before = (await locklift.ton.getBalance(account3.address)).toNumber();
      ...
      wallet3 = await locklift.factory.getAccount('TokenWallet', 'scripts/builds');
      wallet3.setAddress(walletAddress);
      ...
      const eventsBeforeIds = eventsBefore.map(x => x.id);
      const different = eventsAfter.filter(x => !eventsBeforeIds.includes(x.id));
      const decoded = [];
      for (const x of different.filter(x => x.msg_type === 2)) {
        decoded.push(await locklift.ton.client.abi.decode_message_body({
          abi: {
            type: 'Contract',
            value: launchPool.abi
          },
          body: x.body,
          is_internal: false,
        }));
      ...
      const launchPoolParticipationAfter = (await locklift.ton.getBalance(launchPoolParticipation.address)).toNumber();
      const launchPoolAfter = (await locklift.ton.getBalance(launchPool.address)).toNumber();
      const account3After = (await locklift.ton.getBalance(account3.address)).toNumber();
      expect(launchPoolParticipationBefore).to.eq(launchPoolParticipationAfter);
      expect(launchPoolBefore).to.eq(launchPoolAfter);
      expect(account3Before - account3After).to.gt(0.5e9).and.lt(1e9);

      expect(decoded.length).to.eq(1);
      expect(decoded[0].name).to.eq('ClaimRewarded');
      expect(decoded[0].value.user).to.eq(account3.address);
      expect(decoded[0].value.sum).to.eq('50');

      const launchPoolWalletBalanceAfter = await launchPoolWallet.call({ method: 'balance', params: {}});
      const wallet3BalanceAfter = await wallet3.call({ method: 'balance', params: {}});

      expect(launchPoolWalletBalanceBefore - launchPoolWalletBalanceAfter).to.eq(supplyAmount);
      expect(wallet3BalanceAfter.toNumber()).to.eq(supplyAmount);
    });
