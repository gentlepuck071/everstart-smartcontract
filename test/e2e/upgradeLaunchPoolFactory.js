...
describe('upgradeLaunchPoolFactory', async function() {
  describe('upgradeLaunchPoolFactory', async function() {
    ...
    it('Upgrade LaunchPoolFactory', async function() {
      this.timeout(20000);

      const NewLaunchPoolFactory = await locklift.factory.getContract('LaunchPoolFactory');

      const [keyPair] = await locklift.keys.getKeyPairs();

      const launchPoolVersionBefore = await launchPoolFactory.call({
        method: 'version',
        params: {}
      });
      ...
      await account1.runTarget({
        ...
        method: 'setCode',
        params: {
          newCode: NewLaunchPoolFactory.code,
      ...
      const launchPoolVersionAfter = await launchPoolFactory.call({
        method: 'version',
        params: {}
      });
      expect(launchPoolVersionAfter.toNumber() - launchPoolVersionBefore.toNumber()).to.be.equal(1);
    });
