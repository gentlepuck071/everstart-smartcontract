...
program
  .allowUnknownOption()
  .option('-lPP, --launchPoolParticipation <launchPool>', 'LaunchPoolParticipation address')
...
async function main() {
  if (!options.launchPoolParticipation || !options.investorAddress || !options.investorKeyPhrase || !options.callbackId) {
    ...
    console.log(`
      locklift run --config locklift.config.js --network mainnet --script scripts/9-claim-reward.js \
