...
program
  ...
  .option('-tA, --tokenAmount <tokenAmount>', 'amount of nano EVER')
...
async function main() {
  if (!options.launchPool || !options.tokenAmount || !options.ownerAddress || !options.ownerKeyPhrase) {
    ...
    console.log(`
      locklift run --config locklift.config.js --network mainnet --script scripts/10-withdraw-evers.js \
